...
class UzumMarket {
    ...
    updateCartNotification() {
        ...
        if (!cartBadge) {
            ...
            const cartDiv = document.querySelector('.savat');
            if (cartDiv) {
        ...
        this.saveToStorage("cartItems", this.cartItems);
    }
            ...
            card.innerHTML = `
                <div class="card-image-container">
                    <img src="${product.images[0]}" alt="${product.productName}"
                        class="card-image" onerror="this.onerror=null;this.src='https://images.uzum.uz/cuea8jc5j42bjc4bhr1g/original.jpg'">
    ...
    setupEventListeners() {
        ...
        document.addEventListener("click", (e) => {
            ...
            if (e.target.closest(".market-card") && !e.target.closest(".like-btn") && !e.target.closest(".savatcha")) {
    ...
    setupLocationModal() {
        ...
        this.renderLocations = (cities) => {
            ...
            cities.forEach(city => {
                ...
                item.innerHTML = `<span class="location-name">${city}</span>`;
        ...
        const filterLocations = () => {
            ...
            const filtered = this.locations.filter(city => city.toLowerCase().includes(term));
            this.renderLocations(filtered);
        };

        // Event listeners
        dostavka.addEventListener('click', openModal);
...
document.addEventListener("DOMContentLoaded", () => {
    const app = new UzumMarket();

    // If we're on the detail page, render the product details
    if (window.location.pathname.includes('detailist.html')) {
